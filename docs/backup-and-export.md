...
The current filename is `writin-backup-<timestamp>.mybook-backup.json` and the share title is **Writin backup**, consistent with the Writin documentation branding. The browser share API is used when supported, otherwise an ordinary download is triggered. The user/browser decides the save location.
