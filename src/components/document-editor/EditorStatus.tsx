...
export function EditorStatus({ status }: { status: EditorSaveStatus }) {
  ...
  return <span role="status" aria-live="polite" className="inline-flex min-h-8 items-center gap-1.5 text-sm text-muted"><Icon aria-hidden="true" className={`size-4 ${status === 'saving-locally' || status === 'backing-up' ? 'animate-spin' : ''}`} />{label}</span>
}
